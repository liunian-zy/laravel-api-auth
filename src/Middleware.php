...
namespace Liunian\LaravelApiAuth;

use Closure;
use Illuminate\Http\Request;
use Liunian\LaravelApiAuth\Exceptions\AccessKeyException;
use Liunian\LaravelApiAuth\Exceptions\InvalidTokenException;
use Liunian\LaravelApiAuth\Exceptions\SignatureMethodException;
use Liunian\LaravelApiAuth\Signatures\SignatureInterface;
...
class Middleware
{
     ...
     *
     * @return mixed
     * @throws \Liunian\LaravelApiAuth\Exceptions\AccessKeyException
     * @throws \Liunian\LaravelApiAuth\Exceptions\InvalidTokenException
     * @throws \Liunian\LaravelApiAuth\Exceptions\SignatureMethodException
     */
    public function handle($request, Closure $next)
    {
        if ($this->config['status'] === static::STATUS_ON && !$this->is_skip($request)) {

            $token = $request->hasHeader('api-token') ? $request->header('api-token') : false;
            if ($token === false || $token == null) {
                throw new InvalidTokenException('null api-token');
            }
