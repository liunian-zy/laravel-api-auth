...
5. 自定义签名方法 (可选)
    `config/api_auth.php` 中的 `signature_methods` 可以添加自定义的签名类，该类需要继承自 `Liunian\LaravelApiAuth\Signatures\SignatureInterface` 接口 
    ```php
   <?php
    /**
     * User: liunian
    ...
    namespace Liunian\LaravelApiAuth\Signatures;
...
感谢 [qbhy/laravel-api-auth](https://github.com/qbhy/laravel-api-auth)
